...
import { SafeBalance } from '@/interfaces'
...
export async function GET(_: NextRequest, { params }: { params: { chain: string; safeAddress: string } }) {
    // prepare response
    const responseBody = generateResponseBody<SafeBalance[]>()
    ...
    // validate safeAddress
    const safeAddress = params.safeAddress
    if (!safeAddress || !isAddress(safeAddress)) {
        responseBody.error = `'safeAddress' is required and must be a valid evm address`
        return NextResponse.json(responseBody, { status: 400 })
    }
    ...
    const url = `${chainConfig.transactionService.url}/api/v1/safes/${safeAddress}/balances/?trusted=false&exclude_spam=true`
