...
import { SafeCreation } from '@/interfaces'
...
export async function GET(_: NextRequest, { params }: { params: { chain: string; safeAddress: string } }) {
    // prepare response
    const responseBody = generateResponseBody<SafeCreation>()
    ...
    const url = `${chainConfig.transactionService.url}/api/v1/safes/${safeAddress}/creation`
